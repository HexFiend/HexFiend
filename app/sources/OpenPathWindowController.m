...
- (void)controlTextDidChange:(NSNotification *)obj {
	...
	NSString *path = [stringValue stringByExpandingTildeInPath];
	[self updateOKButtonEnabledState];
	if (operationQueue) {
	    [operationQueue cancelAllOperations];
	    NSOperation *fetchIconOp = [[NSInvocationOperation alloc] initWithTarget:self selector:@selector(fetchIconOp:) object:path];

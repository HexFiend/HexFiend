...
Sections can also be renamed. This can be useful, for example, for adding details to elements found within any array. For example, the PNG file format is chunk based, and by using `sectionname` you can rename each chunk by its derived type (e.g., IHDR, IEND, etc.) This also frees up the section value to contain additional information about that element (e.g., the dimensions of the PNG file as found in the IHDR chunk.)
